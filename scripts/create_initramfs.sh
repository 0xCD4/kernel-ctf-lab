...
mkdir -p "$TMP"/{bin,sbin,etc,proc,sys,dev,tmp,root,home/ctf,lib/modules,shared}
...
# Mount shared folder (virtio-9p) if available
mount -t 9p -o trans=virtio,version=9p2000.L shared /shared 2>/dev/null && \
    echo "[+] Shared folder mounted at /shared" || true
